package io.github.jackzemlanicky.paper_plugins;
        ...
        for(Block block : blockList){
            ...
            if(block.getType() != Material.TNT && block.isSolid()){
                ...
                if(Math.random() > 0.5){
                    ...
                    //below code doesnt work on test server
                    ...
                    // here is alt that does
                    // create new block that will be thrown with the explosion, casted to falling block from entity
                    FallingBlock explodedEntity = (FallingBlock) block.getWorld().spawnEntity(block.getLocation(), EntityType.FALLING_BLOCK);
                    // so it doesn't drop an item if it can't land
                    explodedEntity.setDropItem(false);
                    // set the block's velocity
                    explodedEntity.setVelocity(new Vector(xVel, yVel, zVel));
                }
